...
- 异常线索提醒：有效/待加微/已加微/MQL 线索超过 48 小时未跟进。
- 模拟外呼系统和企微/SCRM 回调。
...
- 销售：张三、李四。
- 主管：王主管。
- 60 条不同阶段的模拟线索，覆盖新线索、待外呼、已接通、未接通、有效、待加微、已加微、加微失败、MQL、SQL、无效等状态。

CSV 导入样例包含 10 条新增线索和 2 条重复手机号样例，位于：
...
- 状态流转改为白名单式状态机，主链路贴合作业要求：`NEW -> PENDING_CALL -> CALLED_CONNECTED/VALID -> PENDING_WECHAT -> WECHAT_ADDED -> MQL -> SQL`，禁止 `NEW -> MQL`、`SQL -> NEW`、`WECHAT_FAILED -> MQL` 等不合理跳转。
